...
public class Withdrawal extends Model<Withdrawal> {
    ...
    public List<Withdrawal> getAll(Long uid)
    {
    	return find("select * from withdrawal where uid=?",uid);
    }
    
    public List<Withdrawal> getAll(int page)
    {
    	return find("select * from withdrawal where state = 1 ORDER BY id DESC LIMIT ?,20", page * 20);
    }
    
    public List<Withdrawal> getRecords(int page)
    {
    	return find("select * from withdrawal where state = 2 ORDER BY id DESC LIMIT ?,20", page * 20);
    }
}
