...
public class WxUtils {
    ...
    public static String wxUrl(String url) {
        ...
        String redirectUrl = "http://" + Constants.BASE_URL + "/user/callback?goto=" + url;
    ...
    public static JSONObject getUserToken(String code) {
        ...
        String getOpenIdUrl = url.replace("{appid}", APPID).replace("{secret}", APPSECRET).replace("{code}", code);

        String jsonResult = null;
        try {
            jsonResult = HttpClientUtils.getGetResponse(getOpenIdUrl);
        } catch (Exception e) {
            e.printStackTrace();
        }
